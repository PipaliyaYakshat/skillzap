...
export class ContentService {
  ...
  async inviteUserToGame(
    ...
    for (const userId of normalizedUserIds) {
      ...
      if (inviterRoom && inviterRoom.participants.includes(userId)) {
      ...
      const user = users.find((u) => this.normalizeId(u._id) === userId);
      if (!user?.isOnline) {
        skippedReasons.push(`User ${userId} is not online`);
        continue;
      }

      // All checks passed - create and send invite
  ...
  findPendingInviteForUser(
    ...
    // Find pending invite where user is the invitee (toUserId)
    for (const [inviteId, invite] of this.pendingInvites.entries()) {
      if (
  ...
  async cancelInvite(
    ...
    // Find the pending invite where user is the invitee (toUserId)
    let foundInvite: PendingInvite | null = null;
    let foundInviteId: string | null = null;
